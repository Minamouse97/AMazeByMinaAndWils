...
import edu.wm.cs.cs301.amazebyminaandwils.generation.Factory;
import edu.wm.cs.cs301.amazebyminaandwils.generation.MazeConfiguration;
import edu.wm.cs.cs301.amazebyminaandwils.generation.MazeContainer;
...
import edu.wm.cs.cs301.amazebyminaandwils.generation.Order.Builder;
...
//    protected MazePanel panel ;
    ...
    protected int dx;  // current direction
    ...
    protected int viewdx; // current view direction, more fine grained than (dx,dy)

    protected int viewdy;
    protected int angle; // current viewing angle, east == 0 degrees
    //static final int viewz = 50;
    protected int walkStep; // counter for intermediate steps within a single step forward or backward
    ...
    private static final String TAG = ":MazeControllerLOG:";
    ...
    public MazeController() {
        ...
        Log.v(TAG, "blank mazecontroller constructor called");
        panel = app.getPanel();
        mazeConfig = new MazeContainer();
        app.setConfig(mazeConfig);
    ...
    public MazeController(Order.Builder builder) {
        ...
        DataHolder app = DataHolder.getInstance();
        panel = app.getPanel();
        Log.v(TAG, "builder mazecontroller constructor called");
        mazeConfig = new MazeContainer();
        app.setConfig(mazeConfig);
        factory = new MazeFactory() ;
        filename = null;
...
//        panel = new MazePanel() ;
        DataHolder app = DataHolder.getInstance();
        panel = app.getPanel();
        Log.v(TAG, "filename mazecontroller constructor called");
        mazeConfig = new MazeContainer() ;
        app.setConfig(mazeConfig);
     ...
     */
    public void init() {
        Log.v(TAG, "controller init function reached");
        // special case: load maze from file
        if (null != filename) {
            Log.v(TAG, "controller init function \"if filename is not null\" statement reached");
...
//        notifyViewerRedraw() ;
    ...
    public void switchToGeneratingScreen() {
        Log.v(TAG, "switchToGeneratingScreen is called");
//        assert state == StateGUI.STATE_TITLE : "MazeController.switchToGeneratingScreen: unexpected current state " + state ;
        // switch state and update screen
//        setState(StateGUI.STATE_GENERATING);
        percentdone = 0;
...
//        cleanViews() ;
    ...
    protected void switchToLossScreen() {
        assert state == StateGUI.STATE_PLAY : "MazeController.switchToLossScreen: unexpected current state " + state ;
    ...
    protected void notifyViewerRedraw() {
        ...
        {
            Viewer v = it.next() ;
//			MazePanel mp = panel.getBufferGraphics() ;
...
////			 panel.redraw(g, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle) ;
//            }
            DataHolder app = DataHolder.getInstance();
            panel = app.getPanel();
            v.redraw(panel, state, px, py, viewdx, viewdy, walkStep, Constants.VIEW_OFFSET, rset, angle) ;
...
//                    switchToFinishScreen();
    ...
    public void deliver(MazeConfiguration mazeConfig) {
        Log.v(TAG, "going into the deliver method wooooo!!");
        DataHolder app = DataHolder.getInstance();
...
//        DataHolder app = DataHolder.getInstance();
        driver = app.getRobDriver();
//        driver = getDriver();
        robot = new BasicRobot(this);

        Log.v(TAG, "Driver in deliver method is: " + driver);

        if (robot != null && driver != null) {
            Log.v(TAG, "deliver if statement is reached");
...
//            }
			...
			Log.v(TAG, "ROBOT AND DRIVER ARE NULL NOOOOOO");
        }
    }



    public void runDeliver() throws Exception {
        DataHolder app = DataHolder.getInstance();
        driver.drive2Exit();
//        driver = app.getRobDriver();
//        if (robot != null && driver != null) {
